...
import { readableStreamToString, KlarnaCheckoutError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new KlarnaCheckoutError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new KlarnaCheckoutError(e, e.response?.data)
                }
                if (innerError instanceof KlarnaCheckoutError) {
                    // Got "KlarnaCheckoutError" from the above try block
                    throw innerError;
                }
